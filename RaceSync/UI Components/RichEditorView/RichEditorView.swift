...
fileprivate let DefaultInnerLineHeight: Int = 28

fileprivate class RichEditorWebView: WKWebView {
...
class RichEditorView: UIView, UIScrollViewDelegate, WKNavigationDelegate, UIGestureRecognizerDelegate {
    /// The delegate that will receive callbacks when certain actions are completed.
    weak var delegate: RichEditorDelegate?
    ...
    func isEditingEnabled(handler: @escaping (Bool) -> Void) {
        isContentEditable(handler: handler)
    }
    
    fileprivate func getLineHeight(handler: @escaping (Int) -> Void) {
        if isEditorLoaded {
    ...
    fileprivate func setHTML(_ value: String) {
        if isEditorLoaded {
    ...
    fileprivate func getClientHeight(handler: @escaping (Int) -> Void) {
    ...
    public func setEditorBackgroundColor(_ color: UIColor) {
        runJS("RE.setBackgroundColor('\(color.toHexString())')")
    }
    ...
    public func setTextColor(_ color: UIColor) {
        ...
        runJS("RE.setTextColor('\(color.toHexString())')")
    }
    
    public func setEditorFontColor(_ color: UIColor) {
        runJS("RE.setBaseTextColor('\(color.toHexString())')")
    }
    
    public func setTextBackgroundColor(_ color: UIColor) {
        runJS("RE.prepareInsert()")
        runJS("RE.setTextBackgroundColor('\(color.toHexString())')")
    }
    ...
    public func insertVideo(_ url: String, posterURL: String="", isBase64: Bool=false) {
            // Remember, both poster and src can be base64 encoded
            runJS("RE.prepareInsert()")
            var theJS: String
            if offline == true {
                // Assuming vidURL already in base64
                theJS = "<div><video class='video-js' controls preload='auto'  data-setup='{}'><source src='\(url)'></source></video></div>"
            } else {
                ...
                theJS = "<div><video \(isBase64 ? "id='"+uuid+"'":"") class='video-js' controls preload='auto' data-setup='{}'><source src='\(url)\(isBase64 ? "":"#t=0.01")'></source></video></div>"
                // The time at the end is so that we can grab a thumbnail IF it's a link
            }
            runJS("RE.insertHTML('\(theJS.escaped)')")
        }
    
    public func insertLink(_ url: String, text: String, title: String = "") {
        runJS("RE.prepareInsert()")
        runJS("RE.insertLink('\(url.escaped)', '\(text.escaped)', '\(title.escaped)')")
    }
    ...
    fileprivate var contentEditable: Bool = false {
        didSet {
    ...
    fileprivate func isContentEditable(handler: @escaping (Bool) -> Void) {
        if isEditorLoaded {
